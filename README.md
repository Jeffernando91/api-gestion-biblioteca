# API REST de Gestión de Biblioteca 📚

[cite_start]Proyecto final para la materia de Lenguajes de Programación (6º Semestre)[cite: 1, 3].

## 🛠️ Tecnologías
- [cite_start]Python [cite: 20]
- [cite_start]FastAPI [cite: 20]
- [cite_start]SQLAlchemy [cite: 20]
- [cite_start]SQLite / PostgreSQL [cite: 20]

## 🚀 Integrantes
- Eliseo Abraham Abarca Palma
- Jefferson Fernando Espinoza Orquera
- Melany Noemi Pesantes Yepez
- Henry Pablo Cantos Muñoz
- Brayan Parraga Sanchez
